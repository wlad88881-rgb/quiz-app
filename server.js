...
// ==================== ЛАБОРАТОРИИ (диагностические тренажёры) ====================
...
// Формирует случайный набор случаев для участника: по одной вариации на
// каждый тип неисправности, в случайном порядке.
...
app.post('/api/lab-sessions/:code/submit', async (req, res) => {
  const { participantId: pid, answers } = req.body; // answers: { "0": "faultId", "1": "faultId", ... }
  const data = db.load();
  const session = data.sessions[req.params.code];
  if (!session || session.type !== 'lab') return res.status(404).json({ error: 'Сессия не найдена' });
  const participant = session.participants[pid];
...
app.get('/api/lab-sessions/:code/export', (req, res) => {
  ...
  // Сводка по типам неисправностей: сколько раз ошиблись именно в этом типе
  const faultStats = {};
